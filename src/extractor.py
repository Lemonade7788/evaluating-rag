...
class Extractor:
    def __init__(self, model=llm, schema_config=None, custom_extractor_prompt=None):
        self.llm = ChatOllama(
            model=model, 
            temperature=0
        )
        self.schema = schema_config or {}
        self.custom_extractor_prompt = custom_extractor_prompt

        # If you want a default system prompt
        self.default_prompt = "You are a helpful extraction agent."

    def extract(self, query):
        if self.custom_extractor_prompt:
            template = ChatPromptTemplate.from_template(self.custom_extractor_prompt)
            # Fill with the actual query instead of "example"

            system_prompt = template.format(input=query, schema=self.schema)
        else:
            system_prompt = self.default_prompt
        chain = create_agent(model=self.llm, system_prompt=system_prompt)
...
def extract_properties(prompt, schema_config, custom_extractor_prompt=None):
    ...
    elif 'messages' in extraction_result:
        content = safe_json_parse(extraction_result['messages'][0].content)
...
class PromptCleaner:
    ...
    def clean(self, prompt, return_pk=False, verbose=False):
        """
        ...
        if properties:
            check_and_update_properties(properties, self.retrievers, input_func=None)
...
def create_extractor(schema_path: str = "schema.json", db: SQLDatabase = db_uri):
    schema_config = load_json(schema_path)
    db = SQLDatabase.from_uri(db)
    pre_prompt = """
    Extract and save the relevant entities mentioned 
    in the following passage together with their properties.

    The questions are historical in nature, based on tabular data from the Sarawak Gazette.  

    If a property is not present and is not required in the function parameters,  
    do not include it in the output.  
    If no properties are found, return an empty JSON.
    The output should not contain any JSON markup.

    Examples:

    Input: List all events in August 1971
    Output:
    [
        {{
            "year": ["1971"],
    ...
    Schema (available properties in the database):  
    {schema}
...
if __name__ == "__main__":
    prompter = create_extractor("src/conf/schema.json", "sqlite:///data/sqlrag_db")
